...
const api: ApiMethods = {
    ...
    downloadFile: async (url: any, onProgress?: (progress: number) => void): Promise<any> => {
        try {
            ...
            if (fileExist) {
                // 2. 如果存在，确认是否删除覆盖
                const popRes = await uni.pop_showModal("提示", "本地已有当前文件是否覆盖");
                if (popRes.confirm) {
            ...
            try {
                ...
                const uniFileTempPath_full = roolFolder.data.filePath + "/apps/" + info.appId + "/" + uniFileTempPath_handle;
                const resRootPath = roolFolder.data.filePath + "/files/REOfflineDoc";
                ...
                // 5. 获取压缩包携带信息
                const res_zipComments = await uni.$tool.toPromise((cb: any) => uni.$re.zipGetComments({ filePath: dowloadCopyResPath, password: "@Yr5!Uk9$Bn3*Lp6#Qj8&Mx2" }, cb));
                if (!res_zipComments.data) return { data: null, isSuccess: false, errMsg: "资源压缩包信息获取失败", };

                // 6. 解压文件
                const res_unzip = await uni.$tool.toPromise((cb: any) => uni.$re.unzipFile({ filePath: dowloadCopyResPath, password: "@Yr5!Uk9$Bn3*Lp6#Qj8&Mx2" }, cb));
